...
if(a.result&&$.isArray(a.files))return a.files};a.prototype.add=function(a,b,c){var d=/^file\/(mp4)$/i,e=!1;b.files.map(function(a){a.type.length&&!d.test(a.type)&&(e=!0)});if(e)return layer.msg("\u8bf7\u4e0a\u4f20MP4\u89c6\u9891\u6587\u4ef6\uff01",{icon:0}),!1;TypeScript.logging.log(this.upload_url,TypeScript.ConsoleColors.Blue);c.fileupload("option","url",this.upload_url);var f=c.data("blueimp-fileupload")||c.data("fileupload"),g=f.options;b.context=f._renderUpload(b.files).data("data",b).addClass("processing");
g.filesContainer[g.prependFiles?"prepend":"append"](b.context);f._forceReflow(b.context);f._transition(b.context);b.process(function(){return c.fileupload("process",b)}).always(function(){b.context.each(function(a){$(this).find(".size").text(f._formatFileSize(b.files[a].size))}).removeClass("processing");f._renderPreviews(b)}).done(function(){b.context.find(".start").prop("disabled",!1);!1!==f._trigger("added",a,b)&&(g.autoUpload||b.autoUpload)&&!1!==b.autoUpload&&b.submit()})};return a}(Bootstrap);
c.share_video=e})(pages||(pages={}));
...
(function(c){(function(c){function b(a,b){for(var d=0;d<b.length;d++){var c=b[d],e=$ts("<div>",{class:"col-md-4"});e.appendElement($ts("<img>",{src:c.avatar,class:"img-fluid rounded-circle shadow-lg",style:"width: 24px;"}));var h=$ts("<span>",{style:"font-size:0.7em; color: lightgray"}).display(c.message_time+"<br />"),k=$ts("<span>",{style:"font-size:0.9em;"}).display(c.message);isNullOrUndefined(c.target)?e.appendElement(h).appendElement(k):(c=$ts("<span>").display(c.target.title+"<br />"),e.appendElement(h).appendElement(c).appendElement(k));
