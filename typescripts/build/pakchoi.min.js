...
errorMsg(f.info))});$ts.get("@api:logins",function(f){0==f.code?(d.renderLoginList(f.info),d.loadLoginsDone=!0,d.loadVisitsDone&&d.loadLoginsDone&&layer.close(a)):(layer.closeAll(),errorMsg(f.info))})};a.prototype.renderLoginList=function(d){for(var a=$ts("#login-list"),b,c=0;c<d.length;c++)b=d[c],b=$ts("<span>",{style:"font-size:0.8em;color:lightgray"}).display(b.create_time).appendElement($ts("<span>",{}).display(b.content)),a.appendElement($ts("<li>").display(b))};a.prototype.renderVisitList=function(d){for(var a=
$ts("#visit-list"),b,c=0;c<d.length;c++)b=d[c],b=$ts("<span>",{style:"font-size:0.8em;color:lightgray"}).display(b.time).appendElement($ts("<span>",{}).display($ts("<a>",{href:b.page_url}).display(b.page_url))),a.appendElement($ts("<li>").display(b))};return a}(Bootstrap);c.profile=e})(pages||(pages={}));
...
c.id+'">'+c.description+"</a>\n                                    </p>\n                                </div>                       \n                            </div>");a.appendElement(c)}};return a}(Bootstrap);c.memorials=e})(pages||(pages={}));
...
b,$(this))},getFilesFromResponse:function(d){a.getFilesFromResponse(d)}});$ts("#do-upload").onclick=function(){$("#target").submit()}};a.prototype.stop=function(a,b){TypeScript.logging.log("Stop!",TypeScript.ConsoleColors.Red)};a.prototype.change=function(a,b){TypeScript.logging.log("Changed!",TypeScript.ConsoleColors.Green)};a.prototype.getFilesFromResponse=function(a){var d=a.result.info;0==a.result.code?layer.msg(d,{icon:1},function(){parent.location.reload()}):layer.msg(d,{icon:5},function(){});
if(a.result&&$.isArray(a.files))return a.files};a.prototype.add=function(a,b,c){TypeScript.logging.log(this.upload_url,TypeScript.ConsoleColors.Blue);c.fileupload("option","url",this.upload_url);var d=c.data("blueimp-fileupload")||c.data("fileupload"),e=d.options;b.context=d._renderUpload(b.files).data("data",b).addClass("processing");e.filesContainer[e.prependFiles?"prepend":"append"](b.context);d._forceReflow(b.context);d._transition(b.context);b.process(function(){return c.fileupload("process",
b)}).always(function(){b.context.each(function(a){$(this).find(".size").text(d._formatFileSize(b.files[a].size))}).removeClass("processing");d._renderPreviews(b)}).done(function(){b.context.find(".start").prop("disabled",!1);!1!==d._trigger("added",a,b)&&(e.autoUpload||b.autoUpload)&&!1!==b.autoUpload&&b.submit()})};return a}(Bootstrap);c.share_video=e})(pages||(pages={}));
...
(function(c){(function(c){function b(a,b){for(var d=0;d<b.length;d++){var c=b[d],e=$ts("<div>",{class:"col-md-4"});e.appendElement($ts("<img>",{src:c.avatar,class:"img-fluid rounded-circle shadow-lg",style:"width: 24px;"}));var g=$ts("<span>",{style:"font-size:0.7em; color: lightgray"}).display(c.message_time+"<br />"),h=$ts("<span>",{style:"font-size:0.9em;"}).display(c.message);isNullOrUndefined(c.target)?e.appendElement(g).appendElement(h):(c=$ts("<span>").display(c.target.title+"<br />"),e.appendElement(g).appendElement(c).appendElement(h));
a.appendElement($ts("<div>",{class:"row"}).display(e))}}c.fetchComments=function(a,d,c){void 0===c&&(c=null);$ts.get("@api:load?resource="+a+"&type="+d,function(a){if(0==a.code){a=a.info;var d=$ts("#comment-list");b(d,a);!isNullOrUndefined(c)&&0<a.length&&c(a[a.length-1].id)}})};c.appendComments=b})(c.modules||(c.modules={}))})(webapp||(webapp={}));
