...
a=DOM.InputValueGetter.getSelectedOptions(a);return"boolean"==typeof a?a:0==a.length?null:"string"==typeof a[0]?a[0]:a[0].value};return a}function k(a,b,f){var c=TypeScript.Reflection.$typeof(b),g=c.typeOf,d=g in a?a[g]():null;if("object"!=g||"HTMLCollection"!=c.class&&"NodeListOf"!=c.class){if(c.isArray)return d.doEval(b,c,f);if("function"==c.typeOf)TypeScript.logging.outputEverything&&!isNullOrUndefined(f)&&"HTMLIFrameElement"==TypeScript.Reflection.getClass(f)&&(console.log("Apply a new ready event on iframe:"),
console.log(f)),DOM.Events.ready(b,["interactive","complete"],f);else{if(TypeExtensions.isElement(b))return TypeExtensions.Extends(b);if(isNullOrUndefined(eval)){a=a[c.class];if(isNullOrUndefined(a))throw"Unsupported data type: "+c.toString();return a().doEval(b,c,f)}return d.doEval(b,c,f)}}else return e.Handlers.Shared.HTMLCollection().doEval(b,c,f)}e.StringEval=new e.Handlers.stringEval;e.typeGenericElement=function(a,b){return"string"==typeof a?e.StringEval.doEval(a,null,b):a};e.Static=function(){var a=
e.Handlers.Shared,b=function(b,f){return k(a,b,f)},c=a.string();b.mode=Modes.production;b=f(b,c);b=g(b);b=d(b);return b=h(b)};var n=[":","_"];e.urlSolver=b;e.queryFunction=k})(Internal||(Internal={}));"function"!=typeof String.prototype.startsWith&&(String.prototype.startsWith=function(e){return this.slice(0,e.length)==e});function $imports(e,d,c,b){void 0===d&&(d=DoNothing);void 0===c&&(c=!1);void 0===b&&(b=!1);return(new HttpHelpers.Imports(e,c,b)).doLoad(d)}
...
c.contentDocument;if("function"===typeof b){if(isNullOrUndefined(f)){if($ts.mode==Modes.production){console.warn("Target document object is nothing! Current event will not be handled....");return}if($ts.mode==Modes.debug){console.warn("Target document object is nothing! Current event will not be handled....");console.log("target iframe:");console.warn(c.contentWindow);return}}else TypeScript.logging.outputEverything&&(TypeScript.logging.log("Add Document.ready event handler.",TypeScript.ConsoleColors.Green),
TypeScript.logging.log("document.readyState = "+f.readyState,TypeScript.ConsoleColors.Green));if(-1<a.indexOf(f.readyState))return b();f.addEventListener("DOMContentLoaded",b,!1)}};d.addEvent=c})(e.Events||(e.Events={}))})(DOM||(DOM={}));
